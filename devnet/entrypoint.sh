...
# The 4 tmkms_listener fields must be written with listen_addr LAST — a
# non-empty listen_addr enables validation of the whole block, so setting it
# first is silently rejected while the others are still empty.
# On a unix:// listener the allowlist is neither required nor enforced (the
# 0600 socket is the auth boundary), so TMKMS_ALLOWED_KMS_PUBKEYS is optional.
